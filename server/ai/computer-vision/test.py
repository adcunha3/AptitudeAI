...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'  # Use '3' to suppress all logs, including INFO and WARNING
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Disable oneDNN custom operations
import tensorflow as tf
tf.get_logger().setLevel('ERROR')  # Explicitly set TensorFlow logging to ERROR
import warnings
...
def fix_base64_padding(base64_string):
    """Add padding if necessary to the Base64 string."""
    # Strip any unwanted whitespace
    base64_string = base64_string.strip()
    
    # Add padding if the string's length is not a multiple of 4
...
def process_frame(frame):
    ...
    with mp_face_mesh.FaceMesh(refine_landmarks=True) as face_mesh, mp_pose.Pose() as pose:
        ...
        if face_results.multi_face_landmarks:
            face_landmarks = face_results.multi_face_landmarks[0]  # Only process the first face
...
if __name__ == "__main__":
    try:
        # Your existing code that processes the frame
        input_data = sys.stdin.read()
        input_frame_data = json.loads(input_data).get("frame")
        
        # Fix the Base64 padding
        input_frame_data = fix_base64_padding(input_frame_data)
        ...
        # Log any exception to stderr for debugging
        exc_type, exc_value, exc_tb = sys.exc_info()
        error_message = "".join(traceback.format_exception(exc_type, exc_value, exc_tb))
        print(f"Error in Python script: {error_message}", file=sys.stderr)
